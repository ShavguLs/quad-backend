...
class ReaderUnitTests(TestCase):
    ...
    def test_content_service_optimistic_locking_triggers(self):
        ...
        new_blocks = [
            {'id': 'blk_new', 'type': 'paragraph', 'text': 'New draft'}
        ]
    ...
    def test_content_service_auto_save_throttles_quick_calls(self):
        ...
        blocks = [
            {'id': 'blk_auto', 'type': 'paragraph', 'text': 'Auto save text'}
        ]
        ...
        self.assertEqual(
            ContentVersion.objects.filter(book_content=book_content, version_type='auto').count(),
            1,
        )
    ...
    def test_content_service_cleanup_old_versions(self):
        ...
        for version_number in range(1, 5):
            ContentVersion.objects.create(
                ...
                blocks_snapshot=[
                    {'id': f'blk_{version_number}', 'type': 'paragraph', 'text': 'snapshot'}
                ],
                version_type='auto',
            )

        old_time = timezone.now() - timedelta(days=10)
        ContentVersion.objects.filter(book_content=book_content).update(created_at=old_time)
        ...
        remaining_versions = list(
            ContentVersion.objects.filter(book_content=book_content).values_list('version_number', flat=True)
        )
