...
class Book(BookThemeMixin, models.Model):
    ...
    total_pages = models.PositiveIntegerField(
        default=0,
        help_text='Total number of pages (set after file conversion)'
    )
    ...
    extraction_diagnostics = models.JSONField(
        default=list,
        blank=True,
        help_text='Text extraction warnings and diagnostics'
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=['status', 'created_at'], name='books_status_created_idx'),
            models.Index(fields=['owner', 'status', 'intake_status'], name='books_owner_status_intake_idx'),
            models.Index(fields=['status', 'intake_status', 'intake_updated_at'], name='books_status_intake_upd_idx'),
            # New index for extraction status queries
            models.Index(fields=['extraction_status', 'extraction_updated_at'], name='books_ext_status_upd_idx'),
        ]
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=['book', 'user', 'view_date'],
                name='books_view_unique_per_day'
            )
        ]
...
class DraftElement(models.Model):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['book', 'page_number', 'z_index'], name='draft_el_book_page_z_idx'),
            models.Index(fields=['book', 'element_type'], name='draft_el_book_type_idx'),
        ]
        constraints = [
            models.CheckConstraint(
            ...
            models.CheckConstraint(
                condition=(
                    (Q(element_type='text') & Q(text_content__isnull=False) & Q(image_asset__isnull=True))
                    |
                    (Q(element_type='image') & Q(text_content__isnull=True) & Q(image_asset__isnull=False))
                ),
    ...
    def __str__(self) -> str:
        return f"Draft element {self.pk} ({self.element_type}) on page {self.page_number}"
...
class ExtractedImage(models.Model):
    ...
    xref = models.PositiveIntegerField(
        help_text='PDF xref identifier for the image'
    )
    image = models.ImageField(
        storage=PrivateMediaStorage(),
    ...
    original_ext = models.CharField(
        max_length=10,
        help_text='Original image format (png, jpeg, etc.)'
    )
    ...
    bbox_x0 = models.FloatField(
        ...
        help_text='Position on page: left coordinate'
    )
    bbox_y0 = models.FloatField(
        null=True, blank=True,
        help_text='Position on page: top coordinate'
    )
    bbox_x1 = models.FloatField(
        null=True, blank=True,
        help_text='Position on page: right coordinate'
    )
    bbox_y1 = models.FloatField(
        null=True, blank=True,
        help_text='Position on page: bottom coordinate'
    )
    has_transparency = models.BooleanField(
        default=False,
        help_text='Whether image had mask/transparency applied'
    )
    colorspace = models.PositiveIntegerField(
        default=0,
        help_text='PDF colorspace identifier'
    )
    file_size = models.PositiveIntegerField(
    ...
    def bbox(self) -> Optional[tuple]:
        ...
        if all(v is not None for v in [self.bbox_x0, self.bbox_y0, 
                                        self.bbox_x1, self.bbox_y1]):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                ...
                name='unique_chapter_position_per_book'
            )
        ]
