...
from apps.books.extraction.image_extractor import (
    ImageExtractor,
    ExtractedImage as ExtractedImageData,
)
...
def _resolve_converter_for_book_file(book_file: BookFile):
    converter = _resolve_converter(book_file.mime_type)
    if converter is not None:
        return converter

    filename = (book_file.original_filename or "").lower()
...
def _build_reader_blocks_from_page(
    page_data: dict,
    page_number: int,
    book_id: int,
    render_preference: str = Book.RENDER_PREFERENCE_TEXT,
) -> list[dict]:
    """
    ...
    use_exact_visual = render_preference == Book.RENDER_PREFERENCE_EXACT_VISUAL
    render_mode = "image" if use_exact_visual else ("html" if html_content else "image")
    fallback_image_path = None

    if render_mode == "image":
        if use_exact_visual and not image_data:
            raise ConversionError(
                f"Exact visual render mode requires page image data for page {page_number}."
            )
        if image_data:
            ...
            try:
                ...
                raise ConversionError(
                    "Failed to store rasterized page image for "
                    f"page {page_number} in exact_visual mode."
                ) from exc
...
def _read_file_from_storage(
    book_file: BookFile, draft_id: int, max_retries: int = 3
) -> bytes:
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                logger.error(
                    "File not found after %d attempts: %s", max_retries, file_path
                )
                raise ConversionError(
            ...
            if attempt < max_retries - 1:
                wait_time = 2**attempt
                logger.warning(
                    "Error reading file (attempt %d/%d), retrying in %ds: %s",
                    attempt + 1,
                    max_retries,
                    wait_time,
                    e,
                ...
                continue
            logger.error(
                "Failed to read file after %d attempts for draft %s: %s",
                max_retries,
                draft_id,
                e,
            )
    ...
    raise ConversionError(
        f"Failed to read file: {file_path}. Last error: {last_exception}"
    )
...
def process_book_upload_task(self, book_id: int):
    ...
    try:
        ...
        render_preference = book.reader_render_preference or Book.RENDER_PREFERENCE_TEXT
        book_file = (
            BookFile.objects.filter(book_id=book_id).order_by("-uploaded_at").first()
        )
        if book_file is None:
            raise ConversionError("No uploaded source file available.")

        converter = _resolve_converter_for_book_file(book_file)
        if converter is None:
            raise ConversionError(
                f"Unsupported file type for conversion: {book_file.mime_type}"
            )
        ...
        for page_data in pages:
            try:
                ...
                if render_preference == Book.RENDER_PREFERENCE_EXACT_VISUAL:
                    raise ConversionError(
                        "Exact visual extraction failed while building reader pages: "
                        f"{page_exc}"
                    ) from page_exc
...
def process_draft_intake(self, draft_id):
    ...
    try:
        ...
        book_file = (
            BookFile.objects.filter(book_id=draft_id).order_by("-uploaded_at").first()
        )
        if book_file is None:
            ...
            if book_file.mime_type in {"application/pdf", "application/x-pdf"}:
                ...
                try:
                    ...
                    extraction_result = extraction_engine.extract(
                        temp_path, book_id=draft.id
                    )

                    created_content_pages = (
                        ExtractionToContentService.create_content_from_extraction(
                            draft,
                            extraction_result,
                        )
                    )
                    ...
                    processed_pages = (
                        extraction_result.total_pages or content_pages_created
                    )
                ...
                for page_data in pages:
                    page_number = int(
                        page_data.get("page_number") or content_pages_created + 1
                    )
        ...
        Book.objects.filter(pk=draft_id).update(
            ...
            intake_diagnostics=intake_diagnostics.get("items", [])
...
def publish_book_task(self, book_id: int, user_id: int) -> dict:
    ...
    try:
        ...
        if result.success:
            logger.info(
                f"Publish task completed for book {book_id}: {result.pages_published} pages"
            )
            return {
                "bookId": book_id,
                "status": "published",
            ...
            logger.warning(
                f"Publish task failed for book {book_id}: {result.error_message}"
            )
            return {
                "bookId": book_id,
                "status": "failed",
        ...
        try:
            Book.objects.filter(pk=book_id).update(
                publish_status="failed", updated_at=timezone.now()
            )
        except Exception:
            logger.exception(
                "Failed to mark book %s publish_status=failed after task exhaustion",
                book_id,
            )
    ...
    try:
        ...
        if not validation["valid"]:
            ...
            _update_book_extraction_status(
                book_id=book_id, status="failed", error=error_msg
            )
            return {"status": "failed", "book_id": book_id, "error": error_msg}

        # Log warnings from validation
        if validation["warnings"]:
            results["warnings"].extend(validation["warnings"])
            logger.warning(
                f"Book {book_id} validation warnings: {validation['warnings']}"
            )
        ...
        _update_book_extraction_status(
            book_id=book_id, status="processing", total_pages=total_pages
        )
        ...
        try:
                ...
                try:
                    ...
                    if page_images:
                        ...
                        _save_extracted_images(
                            book_id=book_id, page_num=page_num + 1, images=page_images
                        )
            ...
            calculator = ConfidenceCalculator()
            confidence_summary = calculator.aggregate_pages(
            ...
            try:
                ...
                logger.info(
                    f"Created {content_pages_created} BookContent records for book {book_id}"
                )
                ...
                images_linked = ExtractionToContentService.link_extracted_images(
                    book=book, extraction_result=extraction_result
                )
                content_linked = images_linked > 0

                logger.info(
                    f"Linked {images_linked} images to content blocks for book {book_id}"
                )
        ...
        return {
            "status": "partial",
            "book_id": book_id,
            "total_pages": validation.get("page_count", 0)
            if "validation" in dir()
            else 0,
            "pages_processed": pages_processed,
            "images_extracted": sum(
                len(imgs) for imgs in results.get("images", {}).values()
            ),
        ...
        if self.request.retries < self.max_retries:
            logger.info(
                f"Retrying extraction for book {book_id} (attempt {self.request.retries + 1})"
            )
            raise self.retry(exc=exc, countdown=60)

        # Final failure
        _update_book_extraction_status(
            book_id=book_id, status="failed", error=f"{type(exc).__name__}: {str(exc)}"
        )
...
def _update_book_extraction_status(
    book_id: int,
    status: str,
    error: str = None,
        ...
        diagnostics.append(
            {
                ...
                "metrics": {
                    ...
                    "font_consistency": confidence_summary.get(
                        "font_consistency_average"
                    ),
                    "structure_detection": confidence_summary.get(
                        "structure_detection_average"
                    ),
    ...
    if warnings:
        diagnostics.extend(
            [{"type": "warning", "message": w} for w in warnings[:20]]
        )  # Limit warnings

    if errors:
        diagnostics.extend(
            [{"type": "error", "message": e} for e in errors[:10]]
        )  # Limit errors
...
def _save_extracted_images(book_id: int, page_num: int, images: list):
    """
    ...
    for img_data in images:
        try:
            ...
            existing = ExtractedImage.objects.filter(
                book=book, page_number=page_num, xref=img_data.xref
            ).first()
            ...
            storage_path = image_extractor.save_to_storage(
                img_data, PrivateMediaStorage(), upload_path
            )
